...
def get_cvars(dist_data, alph, sampsizes, k, k_rho, k_alt=None):
    n_cpus = mp.cpu_count()
    pool = mp.Pool(n_cpus)
    ...
    # different sample level for biased POT estimator
    else:
        for d, kr, ka in zip(dist_data, k_rho, k_alt):
            result1 = [pool.apply_async(sample_iter_pot, args=(x, alph, sampsizes, k, True, kr)) for x in d]
            result2 = [pool.apply_async(sample_iter_pot, args=(x, alph, sampsizes, ka, False)) for x in d]
            result3 = [pool.apply_async(sample_iter_sa, args=(x, alph, sampsizes)) for x in d]
            c_est1 = np.array([r.get() for r in result1])
            c_est2 = np.array([r.get() for r in result2])
            c_est3 = np.array([r.get() for r in result3])
...
if __name__ == '__main__':
    ...
    # burr hyperparams
    n_exp = np.load('rho_samp_theta.npy')
    k_b_rho = [(N**i).astype(int)-1 for i in n_exp]

    # frechet hyperparams
    k_f_rho = [N-1]*len(frec_dists)

    # threshold excesses for distributions
    k = np.ceil(N**0.8).astype(int)
    k_alt_exp = [0.6, 0.8, 0.8, 0.8, 0.8]
    k_alt = [np.ceil(N**i).astype(int) for i in k_alt_exp]
    ...
    #burr_result = get_cvars(burr_data, alph, N, k, k_b_rho, k_alt)
